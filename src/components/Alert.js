...
const Alert = ({
  ...
  return (
    <div
      className={`p-4 flex justify-between items-center ${classes} ${fixedPosition[position]} ${colorSchema[theme]}`}
    >
      <span>{message}</span>
      <span className="ml-10 cursor-pointer" onClick={() => close()} onKeyPress={() => close()}>
        <svg
          fill="currentColor"
          className="h-6"
          viewBox="0 0 24 24"
